...
public class Num {
	 ...
	 */
	public static float[] qValueFDR(File tempDirectory, float[] pValues, File fullPathToR, boolean min10Log10Transformed, boolean verbose, float defaultValueIfInf){
		//make random word
		...
		try {
			...
			script.append("qobj = qvalue (s[,1]); s[,1] = qobj$qvalues; ");
			if (min10Log10Transformed) script.append("s[,1] = -10 * log10(s[,1]); ");
			script.append("write.table(s, file='"+rResults.getCanonicalPath()+"',row.names = FALSE, col.names = FALSE, sep = \"\t\") ");
			//write script
			...
			if (rResults.exists() != false){
				pvalues = Num.loadFloats(rResults,defaultValueIfInf);
	...
	public static double[] binomialPValues(double expect, File tempDirectory, int[][] treatObsControlObs, File fullPathToR, boolean min10Log10Transform, boolean multiplyLastPValByRandomNumber){
		//make random word
		String rndWrd = Passwords.createRandowWord(6);
		//write scores to file
		File scores = new File(tempDirectory, rndWrd+"_Scores.txt");
		...
		try {
			...
			if (multiplyLastPValByRandomNumber){
				...
				if (min10Log10Transform) script.append("p = -10 * log10(pbinom(s[,1]-1, s[,1]+s[,2], "+expect+ ", lower.tail=FALSE)); ");
			...
			Double defaultValue;
			if (min10Log10Transform) defaultValue = new Double(3233.666);
			else defaultValue = new Double(4.940666e-324);
			if (rResults.exists() != false){
				pvalues = Num.loadDoubles(rResults, defaultValue);
	...
	public static double[][] binomialPValMatrix (int maxNumOb, double expectation, File tempDirectory, File R, boolean logTransPVals){
		...
		//calc pvals
		double[] pvals = Num.binomialPValues(expectation, tempDirectory, obs, R, logTransPVals, false);
