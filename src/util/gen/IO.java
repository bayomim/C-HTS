...
public class IO {
	...
	public static boolean copyDirectoryRecursive (File sourceDir, File destDir, String extension){
		...
		for (int i=0; i< files.length; i++){
			if (files[i].isDirectory()) copyDirectoryRecursive(files[i], new File (destDir, files[i].getName()), extension);
			else {
	 ...
	 * Returns false if at any time a file cannot be deleted.*/
	...
	public static ArrayList fetchAllFilesRecursively (File directory, String extension){
		ArrayList files = new ArrayList(); 
		File[] list = directory.listFiles();
		for (int i=0; i< list.length; i++){
			if (list[i].isDirectory()) {
				ArrayList al = fetchAllFilesRecursively (list[i], extension);
				files.addAll(al);
			}
			else{
	...
	/**For each base name (ie /home/data/treat1 ), makes a File object using the chromExtenstion
	...
	public static File[] fetchFiles(String[] baseNames, String chromExtension){
		ArrayList al = new ArrayList();
		//for each base name look for an associated file with the chromExtension
	...
	public static File[] fetchFilesRecursively (File directory, String extension){
		...
		ArrayList al = fetchAllFilesRecursively (directory, extension);
		File[] files = new File[al.size()];
		al.toArray(files);
		return files;
	}
		...
		try {
			...
			while (wait){
				...
				if (outputFile!=null){ //check if null again
					...
					if (lastLine.startsWith("Finished!") || lastLine.startsWith("cd: Too many arguments")) wait=false;  //end found exit wait loop
				...
				if (counter>57600000) { //16hrs 57600000
					System.out.println("\n    Error: shell script failed to return from qsub after "+counter/60000+
							" minutes.\nFind and kill the job: (name Nix, process number is the last digits after the .o in -> "+outputFile.getName());
					System.exit(1);	
				}
	 ...
	 * */
	public static HashMap loadFileIntoHashMap(File file){
		HashMap names = new HashMap(1000);
		try{
			BufferedReader in = new BufferedReader(new FileReader(file));
			String line;
			String[] keyValue;
	...
	public static String makeFullPathName(String resultsDirectory, String fileName){
		...
		try{
			...
			System.out.println("Problem building name to write files!");
			e.printStackTrace();
		}
		return fullPathName;
	}

	/**Returns a HashMap of the file name sans extension and the file.
	 * Returns null if duplicate name is found.*/
	...
	public static HashMap parseFile (File file, int index, boolean ignoreDuplicateKeys){
		...
		try{
			...
			while ((line=in.readLine()) != null){
				...
				else if (al.containsKey(item)) return null;
	 ...
	 * If so it converts the file name dividers to unix.
